...
Set `prompt.queue_edit` to `true` to edit unconsumed prompts in the active session with the previous-history key. It defaults to `false`. See [queued prompt editing](/docs/tui#queued-prompt-editing).
