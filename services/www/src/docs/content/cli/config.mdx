...
Configure mouse capture, scrolling, editor context, pastes, prompt image previews, and queued prompt editing:
...
| `prompt.queue_edit`        | boolean                         | Lets `Up` on an empty prompt withdraw queued prompts for editing.           |

</div>

With `prompt.queue_edit` enabled, press `Up` on an empty prompt to pull every prompt that is still queued for the active session back into the composer, in order and with attachments, while the active turn continues. Press `Enter` to send the edited draft as one prompt. When nothing is queued, `Up` recalls prompt history as usual. If you type, switch sessions, or close the prompt while a withdrawal is pending, the returned prompts are saved to the prompt stash instead of replacing your draft.
